	...
	Describe("when Health server is ready to serve RESTful API", func() {
		...
		Context("when a request to query health comes", func() {
			It("returns with a 200", func() {
				...
				Expect(healthData).To(ContainSubstring("autoscaler_apiserver_concurrent_http_request"))
				Expect(healthData).To(ContainSubstring("autoscaler_apiserver_policyDB"))
				Expect(healthData).To(ContainSubstring("autoscaler_apiserver_bindingDB"))
